...
function bindHtml() {
  ...
  cartList.forEach(item => {
    str += `
      <li class="cart-item">
        <div class="left">
          <input ${item.is_select === 1 ? 'checked' : ''} id="select" data-id="${item.Id}" type="checkbox">
        </div>
        <div class="right">
          <div class="media">
            <div class="media-left">
              <a href="#">
                <img class="media-object" src="https://img01.hua.com/uploadpic/newpic/${item.ItemCode}.jpg_220x240.jpg" alt="...">
              </a>
            </div>
            <div class="media-body">
              ...
              <div class="btn-group pull-right" role="group" aria-label="...">
                <button data-id="${item.Id}" type="button" class="btn btn-default" id="sub" ${item.cart_number - 0 === 1 ? 'disabled' : ''}>-</button>
                <button type="button" class="btn btn-default" disabled>${item.cart_number}</button>
                <button data-id="${item.Id}" type="button" class="btn btn-default" id="add" ${item.cart_number - 0 >= item.Price ? 'disabled' : ''}>+</button>
              </div>
              <button id="del" data-id="${item.Id}" class="btn btn-danger">删除</button>
            </div>
          </div>
        </div>
      </li>
